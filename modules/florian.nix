{ pkgs, lib, homes, ... }:

{
  # Install default managed home folder
  home-manager.users.florian = lib.mkDefault homes.terminal-solarized;

  # Setup sytem user account
  users.users.florian = {
    isNormalUser = true;
    uid = 1000;
    description = "Florian Franzen";
    extraGroups = [ "wheel" ];
    shell = pkgs.zsh;
    openssh.authorizedKeys.keys = [
      "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIBTdvxKKQnoCUpsvBfUWYHDwA2tMpFj3zwT2s3kfuWWO mind-the-gap"
      "ssh-rsa  cardno:000604936884"
    ];
  };
}	
